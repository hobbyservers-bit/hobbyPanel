...
import { deleteFiles } from "@/lib/wings/client";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { root, files } = (await req.json()) as { root: string; files: string[] };
  if (!Array.isArray(files) || !files.length) {
    return NextResponse.json({ error: "files array required" }, { status: 400 });
  }

  try {
    await deleteFiles(nodeCreds(access.server.node), access.server.externalId, root ?? "/", files);
